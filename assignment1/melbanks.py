...
class LogMelFilterBanks(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # Compute complex spectrogram → power spectrum
        spec = self.spectrogram(x)                        # (batch, n_freqs, n_frames)
        power_spec = torch.abs(spec) ** self.power        # (batch, n_freqs, n_frames)

        # Apply mel filterbank: (n_mels, n_freqs) @ (batch, n_freqs, n_frames) → (batch, n_mels, n_frames)
        mel_spec = torch.matmul(self.mel_fbanks.T, power_spec)
